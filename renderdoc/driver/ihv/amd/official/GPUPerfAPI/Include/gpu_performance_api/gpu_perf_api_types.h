...
{
    GpaUInt32 gpuBlockId;        ///< GPU block identifier.
    GpaUInt32 gpuBlockInstance;  ///< GPU block instance.
    GpaUInt32 dataOffset;        ///< Offset from the start of counterDeltaValues for the deltas for this counter instance.
    GpaUInt32 eventIndex;        ///< Event index.
} GpaSpmCounterInfo;

/// GPA SPM data
typedef struct GpaSpmData
{
    GpaUInt32                numberOfTimestamps;           ///< Number of timestamps.
    GpaUInt32                numberOfSpmCounterInfo;       ///< Number of SpmCounterInfo structs.
    GpaUInt32                numberOfCounterData;          ///< Number of CounterData values.
    GpaUInt32                numberOfBytesPerCounterData;  ///< Number of bytes for each CounterData entry.
    const GpaUInt64*         pTimestamps;                  ///< Array of numberOfTimestamps number of timestamps.
    const GpaSpmCounterInfo* pSpmCounterInfo;              ///< Array of numberOfSpmCounterInfo number of SpmCounterInfo.
    const GpaUInt16*         pCounterData16bit;            ///< Array of numberOfSpmCounterInfo * numberOfTimestamps counter delta values (may be 16 or 32-bit values).
} GpaSpmData;
#endif
